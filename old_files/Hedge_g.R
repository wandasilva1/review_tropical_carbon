...
#data
#importing UGS/urban intensity/vegetation data
...
storage <- storage %>%
  filter(!str_detect(Habitat, "Gardens"))
...
(habitat <- ggplot(resultados_all, aes(x = reorder(paste(group1, "-", group2), yi),
                           y = yi,
                           color = Tipo,
                           shape = Tipo)) +
...
#data
#importing habitat/urban intensity/vegetation data
data <- read_excel("tropical_9_1_2025.xlsx", 
                   sheet = "all_data_organized")
#numeric
data[ , c(10, 11, 29:32,34:41,43,44,46,47)] <- lapply(data[ , c(10, 11, 29:32,34:41,43,44,46,47)], as.numeric)
...
(urbanc <- ggplot(resultados_all2, aes(x = reorder(paste(group1, "-", group2), yi),
                           y = yi,
                           color = Tipo,
                           shape = Tipo)) +
  geom_point(size = 3, position = position_dodge(width = 0.6)) +
...
#Assumption tests
# Normal test
by(log(storage$`C_area_ton/ha`), storage$pop_classes, shapiro.test)

# Homogeneity of variances
lev <- leveneTest(log(`C_area_ton/ha`) ~ pop_classes, data = storage)
lev
...
# Generating all possible comparisons
pairs2 <- combn(unique(resumo2$pop_classes), 2, simplify = FALSE)

# Calculating Hedges’ g for each pair
resultados2 <- map_dfr(pairs2, function(par) {
  g1 <- resumo2 %>% filter(pop_classes == par[1])
  g2 <- resumo2 %>% filter(pop_classes == par[2])
...
# Chart
ggplot(resultados2, aes(x = reorder(paste(group1, "-", group2), yi),
                        y = yi)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high),
                width = 0.2) +
...
global_pop <- rma.mv(yi, vi, random = ~ 1 | group1/group2, data = resultados2)
summary(global_pop)
...
#Assumption tests
# Normal test
by(log(sequestration$`Carbon_ton/year/ha`), sequestration$pop_classes, shapiro.test)

# Homogeneity of variances
lev <- leveneTest(log(`Carbon_ton/year/ha`) ~ pop_classes, data = sequestration)
lev

# Statistics by habitat type:
resumo2_s <- sequestration %>%
  group_by(pop_classes) %>%
  summarise(
    mean = mean(`Carbon_ton/year/ha`, na.rm = TRUE),
    sd = sd(`Carbon_ton/year/ha`, na.rm = TRUE),
...
# Generating all possible comparisons
pairs2_s <- combn(unique(resumo2_s$pop_classes), 2, simplify = FALSE)

# Calculating Hedges’ g for each pair
resultados2_s <- map_dfr(pairs2_s, function(par) {
  g1 <- resumo2_s %>% filter(pop_classes == par[1])
  g2 <- resumo2_s %>% filter(pop_classes == par[2])
...
# Chart
ggplot(resultados2_s, aes(x = reorder(paste(group1, "-", group2), yi),
                        y = yi)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = ci_low, ymax = ci_high),
                width = 0.2) +
...
global_pop_s <- rma.mv(yi, vi, random = ~ 1 | group1/group2, data = resultados2_s)
summary(global_pop_s)


## ## merging the results for city-size ## ##

resultados2$Tipo <- "Carbon storage"
resultados2_s$Tipo <- "Carbon sequestration"

resultados_all3 <- rbind(resultados2, resultados2_s)

## plot city ------------------------------------------------------------------

(city <- ggplot(resultados_all3, aes(x = reorder(paste(group1, "-", group2), yi),
                            y = yi,
                            color = Tipo,
                            shape = Tipo)) +
  geom_point(size = 3, position = position_dodge(width = 0.6)) +
...
ggsave("box_meta.png", plot = box_h, dpi = 300, width = 11, height = 9, units = "in")
...
write.xlsx(df_export1, 
           file = "results_urban.xlsx",
           sheetName = "Comparacoes",
           rowNames = FALSE)


df_export2 <- resultados2 %>%
  mutate(
    yi = round(yi, 3),
    ci_low = round(ci_low, 3),
    ci_high = round(ci_high, 3),
...
write.xlsx(df_export2, 
           file = "results_city.xlsx",
           sheetName = "Comparacoes",
           rowNames = FALSE)
...
write.xlsx(df_export1_s, 
           file = "results_urban_s.xlsx",
           sheetName = "Comparacoes",
           rowNames = FALSE)


df_export2_s <- resultados2_s %>%
  mutate(
    yi = round(yi, 3),
    ci_low = round(ci_low, 3),
    ci_high = round(ci_high, 3),
...
write.xlsx(df_export2_s, 
           file = "results_city_s.xlsx",
           sheetName = "Comparacoes",
           rowNames = FALSE)
