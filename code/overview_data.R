...
length(data_p$Title)#72 papers - 93 papers
length(unique(data_p$Region_Country))#26 countries - 28 countries
...
#counting number of cities per climate
clic <- data %>%
  group_by(Climatic_region) %>%
  summarise(n_city = n_distinct(City),
            p_city = n_city/101)

#counting number of cities per biome
bioc <- data %>%
  group_by(Biome) %>%
  summarise(n_city = n_distinct(City),
            p_city = n_city/101)
...
#pluviosity
mean(data$`Pluviosity(average annual rainfall-mm)`, na.rm = T)#1588.75 - 1547.062
sd(data$`Pluviosity(average annual rainfall-mm)`, na.rm = T)#829.1909 - 756.7415

#temperature
mean(data$`Temperature(average annual temperature-°C)`, na.rm = T)#24.13 - 24
sd(data$`Temperature(average annual temperature-°C)`, na.rm = T)#3.97 - 4
...
(seu <- sd(df_urban$`C_area_ton/ha`)/sqrt(length(df_urban$`C_area_ton/ha`)))
#RURAL
df_rural <- df_rural %>%
  filter(!is.na(`C_area_ton/ha`))
mean(df_rural$`C_area_ton/ha`, na.rm = T)#102.32
sd(df_rural$`C_area_ton/ha`, na.rm = T)#66.46
(ser <- sd(df_rural$`C_area_ton/ha`)/sqrt(length(df_rural$`C_area_ton/ha`)))
#PERIURBAN
df_peri <- df_peri %>%
  filter(!is.na(`C_area_ton/ha`))
mean(df_peri$`C_area_ton/ha`, na.rm = T)#80.44
sd(df_peri$`C_area_ton/ha`, na.rm = T)#102.12
(sep <- sd(df_peri$`C_area_ton/ha`)/sqrt(length(df_peri$`C_area_ton/ha`)))
#URB-RUR
df_urb_ru <- df_urb_ru %>%
  filter(!is.na(`C_area_ton/ha`))
mean(df_urb_ru$`C_area_ton/ha`, na.rm = T)#125.25
sd(df_urb_ru$`C_area_ton/ha`, na.rm = T)#150.41
(sep <- sd(df_urb_ru$`C_area_ton/ha`)/sqrt(length(df_urb_ru$`C_area_ton/ha`)))
...
tabela_carbono <- data %>%
  ...
  summarise(
    ...
    # Carbono por área
    Media_C_area = mean(`C_area_ton/ha`, na.rm = TRUE),
    Min_C_area = min(`C_area_ton/ha`, na.rm = TRUE),
    Max_C_area = max(`C_area_ton/ha`, na.rm = TRUE),
    
    # Carbono por área por ano
    Media_C_area_ano = mean(`Carbon_ton/year/ha`, na.rm = TRUE),
    Min_C_area_ano = min(`Carbon_ton/year/ha`, na.rm = TRUE),
    Max_C_area_ano = max(`Carbon_ton/year/ha`, na.rm = TRUE),
    
    .groups = "drop"
  )%>%
  mutate(
    `Carbon storage (Mg C ha-¹)` = sprintf(
      "%.2f (%.2f–%.2f)",
      Media_C_area, Min_C_area, Max_C_area
    ),
    
    `Carbon sequestration (Mg C ha-¹ year-¹)` = sprintf(
      "%.2f (%.2f–%.2f)",
      Media_C_area_ano, Min_C_area_ano, Max_C_area_ano
    )
  ) %>%
  select(
    Variable,
...
write.xlsx(tabela_carbono, 
           file = "carbon_variables.xlsx",
           sheetName = "table",
           rowNames = FALSE)
